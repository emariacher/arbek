...
package fogus.baysick {
  ...
  class Baysick extends App {
    ...
    implicit def int2LineBuilder(i: Int): LineBuilder = LineBuilder(i)
    implicit def toAppendr(key:Any): Appendr = Appendr(key)
    implicit def symbol2Assignment(sym:Symbol): Assignment = Assignment(sym)
    implicit def symbol2BinaryRelation(sym:Symbol): BinaryRelation = BinaryRelation(() => binds.num(sym))
    implicit def fnOfInt2BinaryRelation(fn:Function0[Int]): BinaryRelation = BinaryRelation(fn)
    implicit def symbol2MathFunction(sym:Symbol): MathFunction = MathFunction(() => binds.num(sym))
    implicit def fnOfInt2MathFunction(fn:Function0[Int]): MathFunction = MathFunction(fn)
  }
}
