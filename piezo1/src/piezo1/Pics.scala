...
class Pics(l: List[(Double, Int)], ttick: Duration, bpm: Double, decimation: Int) {
    ...
    myPrintIt(ttick, "%3.2f".format(bpm), numTicks4OneBpm)

    val goodSampleRoots = sliding2.filter(z => within(z._2, numTicks4OneBpm, 0.07)).map(z => (z._1._2,z._2))
    myPrintIt(goodSampleRoots.mkString("\n"))
