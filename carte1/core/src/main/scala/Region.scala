...
class Region(val tbx: Tableaux, val canton: Canton) {
  val id: Int = tbx.cb.lregions.lr.size
  var voisins = ListSet[Region]()
  var cantons = ListSet[Canton](canton)
  var couleur = Color.white
  var couleursDejaEssayees: ListSet[Color] = ListSet[Color]()
...
object Couleurs {
  var l: ListSet[Color] = ListSet(Color.yellow, Color.red, Color.green, Color.cyan)
}
