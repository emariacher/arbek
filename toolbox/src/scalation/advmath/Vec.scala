...
object Vec {
    ...
    implicit def canBuildFrom[A: Numeric: ClassTag]: CanBuildFrom[Vec[A], A, Vec[A]] =
    ...
    def newBuilder[A: Numeric: ClassTag]: Builder[A, Vec[A]] = new ArrayBuffer[A] mapResult fromSeq[A]

    /*::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
    /** Build a numeric vector from a numeric sequence.
     */
    def fromSeq[A: Numeric: ClassTag](buf: Seq[A]): Vec[A] = new Vec(buf.toArray)

    /*::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
    /** Build a numeric vector from a numeric sequence.
     */
    def apply[A: Numeric: ClassTag](values: A*) = fromSeq(values)

    /*::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
    /** Build a numeric vector of certain length.
     */
    def ofLength[A: Numeric: ClassTag](length: Int) = new Vec(Array.ofDim(length))

    /*::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
    def fill[A: Numeric: ClassTag](dim: Int)(f: => A): Vec[A] = fromSeq(Array.fill(dim)(f).toSeq)

    /*::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
    def one[A: Numeric: ClassTag](j: Int, dim: Int): Vec[A] = {
 ...
 */
class Vec[A: Numeric: ClassTag] protected (x: Array[A])
    ...
    def compare(a: Vec[A], b: Vec[A]): Int = {
        for (i <- a.range) {
            if (a(i).toDouble > b(i).toDouble)  1
            if (a(i).toDouble < b(i).toDouble) -1
        }
        0
    }
    ...
    def min(rhs: Vec[A]): Vec[A] = {
        val values = for (i ← range) yield if (rhs(i).toDouble < x(i).toDouble) rhs(i) else x(i)
        Vec.fromSeq(values)
    } // min
    ...
    {
        var j = k
        for (i <- k + 1 until length if k != l && x(i).toDouble < x(j).toDouble) j = i
        j
    } // argmin
    ...
    def argminNeg (k: Int = 0, l: Int = -1): Int =
    {
        val _0 = nu.zero
        val j = argmin (k, l)
        if (x(j).toDouble < _0.toDouble) j else -1
    } // argmaxPos
    ...
    {
        val _0 = nu.zero
        for (i <- k until length if x(i).toDouble < _0.toDouble) return i
        -1
    } // firstNeg
