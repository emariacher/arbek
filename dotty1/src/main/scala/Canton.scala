...
class Canton(val ct: CantonType, val c: Carre) {
  var r: Region = _
  val lcarres: ListCarres = tbx.cb.lcarres
    ...
    lscv.filter(_.r == null).filter(_.c == c).foreach((canton: Canton) => {
      ...
      ct.i match {
        case NORDOUEST => canton.ct.i match {
          ...
          case NORDEST => if (!c.frontieres.contains(nord)) {
            canton.r = r; canton.repandVoisin(i + 1)
          }
          case SUDOUEST => if (!c.frontieres.contains(ouest)) {
            canton.r = r; canton.repandVoisin(i + 1)
          }
          case SUDEST =>
        }
        case NORDEST => canton.ct.i match {
          case NORDOUEST => if (!c.frontieres.contains(nord)) {
            canton.r = r; canton.repandVoisin(i + 1)
          }
          case NORDEST => require(false)
          case SUDOUEST =>
          case SUDEST => if (!c.frontieres.contains(est)) {
            canton.r = r; canton.repandVoisin(i + 1)
          }
        }
        case SUDOUEST => canton.ct.i match {
          case NORDOUEST => if (!c.frontieres.contains(ouest)) {
            canton.r = r; canton.repandVoisin(i + 1)
          }
          case NORDEST =>
          case SUDOUEST => require(false)
          case SUDEST => if (!c.frontieres.contains(sud)) {
            canton.r = r; canton.repandVoisin(i + 1)
          }
        }
        case SUDEST => canton.ct.i match {
          case NORDOUEST =>
          case NORDEST => if (!c.frontieres.contains(est)) {
            canton.r = r; canton.repandVoisin(i + 1)
          }
          case SUDOUEST => if (!c.frontieres.contains(sud)) {
            canton.r = r; canton.repandVoisin(i + 1)
          }
