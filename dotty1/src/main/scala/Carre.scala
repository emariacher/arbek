...
class Carre {
  ...
  var row: Int = _
  var col: Int = _
  var frontieres: List[Frontiere] = List[Frontiere]()
  ...
  var cantons: List[Canton] = List[Canton](cno, cne, cso, cse)
  val p: (Int, Int) = lcarres.getNextRowCol
  //			// get row col
  row = p._1
  col = p._2
  // get Frontieres
  val leftCarre = getLeftCarre
  val upCarre: Option[Carre] = getUpCarre
  leftCarre match {
