...
class CheckEnglishLanguageMostCurrentCombinations2(val i_ghost: Int, ts_quartets: TreeSet[Quartet]) {
	...
	Quartet.combinations(3, "A,S,D,F".split(",").toList).foreach((l: List[String]) => l_combiASDF += l.reduceLeft(_ + _))
	...
	Quartet.combinations(3, "Q,W,E,A,S,D".split(",").toList).foreach((l: List[String]) => l_combiQWEASD += l.reduceLeft(_ + _))
	var l_combiWERSDF = ListSet.empty[String];
	Quartet.combinations(3, "W,E,R,S,D,F".split(",").toList).foreach((l: List[String]) => l_combiWERSDF += l.reduceLeft(_ + _))
