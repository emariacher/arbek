...
object CheckEnglishLanguageMostCurrentCombinations {
    ...
    final val l_codes = (KeyDoc.m_keyref.filter((p:(Int, key)) => 
    "Q,W,E,R,T,Z,U,I,O,P,A,S,D,F,G,H,J,K,L,Y,X,C,V,B,N,M".split(",")
    .contains(p._2.s_key)).map((p:(Int, key)) => p._1)++List(41,61)).toList;
...
class CheckEnglishLanguageMostCurrentCombinations(val i_numberOfKeysInAQuartetThatMakeAGhostCondition: Int, ts_quartets: TreeSet[Quartet]) {
	...
	L.myPrintln("*f_total*"+CheckEnglishLanguageMostCurrentCombinations.f_total+
			...
			if(ts_occurrences.until(("",0.5f)).isEmpty) {
				L.myErrPrintln(ts_occurrences.until(("",0.02f)).mkString("Natural English language triplet Occurence\n(triplet,Occurence in %)\n  ","\n  ","\n"))
			} else {
				L.myPrintln(ts_occurrences.until(("",0.02f)).mkString("Natural English language triplet Occurence\n(triplet,Occurence in %)\n  ","\n  ","\n"))
				L.myErrPrintln(ts_occurrences.until(("",0.5f)).mkString("Natural English language triplet Occurence\n(triplet,Occurence in %)\n  ","\n  ","\n"))
			}
	def getOccurrence(s: String): Float = {
			L.myPrintln("["+s+"]")
			val f_sum = permutations(s).map(getOccurrenceByPermutation(_)).reduceLeft[Float](_+_)
			L.myPrintln("  sum["+s+"]="+f_sum+" "+(f_sum*100/CheckEnglishLanguageMostCurrentCombinations.f_total))
	...
	def getOccurrenceByPermutation(s: String): Float = {
			...
			if(b.size>1) {
				b.foreach((b1: String) => L.myPrintln("     *10*"+b1.substring(0,Math.min(20,b1.size))))
				val f_sum = b.tail.map(getNextFloat(_)).reduceLeft[Float](_+_)
				L.myPrintln("    *sum["+s+"]="+f_sum)
				f_sum
			} else {
				0
			}
