...
class KbdMatrix {
    ...
    def printMatrices() {
        val m_matrixGroupByType = ts_keys.groupBy((k: KbdKey) => k.i_type)
                m_matrixGroupByType.keySet.toList.sorted.foreach((t:Int) => myPrint(t))
    }
    ...
    def CGSprintMatrices() {
        ...
        m_matrixGroupByType.keySet.toList.sorted.foreach((t:Int) => myCGSPrint(t, cgsfile))
    }
    ...
    def translate(s_code: String): Int = {
            ...
            case _: Throwable => KbdKey.i_invalidCode
            }
    }
