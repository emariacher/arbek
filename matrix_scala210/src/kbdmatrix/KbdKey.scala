...
class KbdKey(val i_row: Int, val i_col: Int, val i_type:Int, val i_code:Int) {
	def this(i_row: Int, i_col: Int, i_code:Int) = this(i_row, i_col, KbdKey.i_mainMatrix, i_code)
			def this(i_row: Int, i_col: Int) = this(i_row, i_col, KbdKey.i_mainMatrix, KbdKey.i_emptyCode)
			def this(i_code:Int) = this(0xff, 0xff, KbdKey.i_mainMatrix, i_code)
	...
	def generateQuartets(ts_sameRow: TreeSet[KbdKey],ts_keysMain: TreeSet[KbdKey]): ListSet[Quartet] = {
			var ts_nextKeys = ts_sameRow.from(this).tail.filter(_.isValid || this.isValid)
					...
					.foreach((k: KbdKey) => l_quartets += new Quartet(this, k))
					val l_quartetsSup2 = l_quartets.filter(_.size>2)
					L.myPrintln("N "+toString+" Quartets "+l_quartetsSup2)
					l_quartetsSup2
	}
	...
	def generateQuartetsOld(): ListSet[Quartet] = {
			var ts_nextKeys = km.ts_keys.filter(_.i_type==KbdKey.i_mainMatrix).from(this).tail.filter(_.isValid || this.isValid)
					...
					.foreach((k: KbdKey) => l_quartets += new Quartet(this, k))
					val l_quartetsSup2 = l_quartets.filter(_.size>2)
					L.myPrintln("O "+toString+" Quartets "+l_quartetsSup2)
					l_quartetsSup2
	}
	...
	def tocgsstring(): String = {
			if(i_code==KbdKey.i_emptyCode) {
				"\"Key[Row"+i_row+"][Col"+i_col+"].Type\" = \"None\"\n\"Key[Row"+i_row+"][Col"+i_col+"].TranslationCode\" = 0 # "+KeyDoc.m_keyref(i_code).s_key+"\n\n"
			} else {		
				val kfn = km.findFn(i_row, i_col)
						km.findFn(i_row, i_col) match {
						case Some(kfn: KbdKey) => "\"Key[Row"+i_row+"][Col"+i_col+"].Type\" = \"Function Lock Dependent\"\n\"Key[Row"+i_row+"][Col"+i_col+"].TranslationCode\" = "+
								KeyDoc.m_keyref(i_code).getHex+" # "+KeyDoc.m_keyref(i_code).s_key + " Fn-> "+KeyDoc.m_keyref.getOrElse(kfn.i_code, kfn.i_code)+"\n\n"
						case _ => "\"Key[Row"+i_row+"][Col"+i_col+"].Type\" = \"Standard\"\n\"Key[Row"+i_row+"][Col"+i_col+"].TranslationCode\" = "+
								KeyDoc.m_keyref(i_code).getHex+" # "+KeyDoc.m_keyref(i_code).s_key+"\n\n"
				}
			}
	}
