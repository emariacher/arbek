...
<h2>traces by <a HREF="mailto:emariacher@netcourrier.com">Eric Mariacher</a></h2>
  ...
  <tr><th colspan="3"><xsl:value-of select="trace/ts"/></th></tr>
  <tr><td colspan="3"><xsl:value-of select="trace/header"/></td></tr>
  <xsl:for-each select="trace/footer">
   ...
   <xsl:choose>
    ...
    <xsl:otherwise>
     <td colspan="3" bgcolor="#e0f0e0"><xsl:value-of select="."/></td>
    </xsl:otherwise>
   </xsl:choose>
