...
class Quartet(km: KbdMatrix, i_type: Int, k_topleft: KbdKey, k_botright: KbdKey) {
	...
	def this(km: KbdMatrix, k_topleft: KbdKey, k_botright: KbdKey) = this(km, KbdKey.i_mainMatrix,
	...
	def update(i_row: Int, i_col: Int) {
		...
		case Some(k) => if(k.isValid) ts_quartet += k
		case _ => 
		}
	}
	...
	def getAllStrings(): ListSet[String] = {
			...
			var l_strings = ListSet.empty[String]
			                              ...
			                              km.L.myPrintln("  *2*"+toString+"---"+l_strings)
			                              l_strings
	}
