...
object CheckEnglishLanguageMostCurrentCombinations {
...
class CheckEnglishLanguageMostCurrentCombinations(km: KbdMatrix,
	...
	km.L.myPrintln("*f_total*"+CheckEnglishLanguageMostCurrentCombinations.f_total+
			...
			if(ts_occurrences.until(("",0.5f)).isEmpty) {
				km.L.myErrPrintln(ts_occurrences.until(("",0.02f)).mkString("Natural English language triplet Occurence\n(triplet,Occurence in %)\n  ","\n  ","\n"))
			} else {
				km.L.myPrintln(ts_occurrences.until(("",0.02f)).mkString("Natural English language triplet Occurence\n(triplet,Occurence in %)\n  ","\n  ","\n"))
				km.L.myErrPrintln(ts_occurrences.until(("",0.5f)).mkString("Natural English language triplet Occurence\n(triplet,Occurence in %)\n  ","\n  ","\n"))
			}
	def getOccurrence(s: String): Float = {
			km.L.myPrintln("["+s+"]")
			val f_sum = permutations(s).map(getOccurrenceByPermutation(_)).reduceLeft[Float](_+_)
			km.L.myPrintln("  sum["+s+"]="+f_sum+" "+(f_sum*100/CheckEnglishLanguageMostCurrentCombinations.f_total))
			f_sum*100/CheckEnglishLanguageMostCurrentCombinations.f_total
	}

	def getOccurrenceByPermutation(s: String): Float = {
			km.L.myPrintln("  *["+s+"]")
			val b = CheckEnglishLanguageMostCurrentCombinations.s_dico.split(s)
			if(b.size>1) {
				b.foreach((b1: String) => km.L.myPrintln("     *10*"+b1.substring(0,Math.min(20,b1.size))))
				val f_sum = b.tail.map(getNextFloat(_)).reduceLeft[Float](_+_)
				km.L.myPrintln("    *sum["+s+"]="+f_sum)
				f_sum
			} else {
				0
			}
	...
	def permutations(s_in: String): ListSet[String] = {
			var ls = ListSet.empty[String];
			val zero = s_in.substring(0,1)
			val un = s_in.substring(1,2)
			val deux = s_in.substring(2,3)
			ls += s_in
			ls +=zero+deux+un
			ls +=un+zero+deux
			ls +=un+deux+zero
...
class CompareOccurence extends Ordering[(String, Float)] {
	def compare(m2: (String, Float), m1: (String, Float)) = {
		val i_fdiff = java.lang.Float.floatToIntBits(m1._2).compare(java.lang.Float.floatToIntBits(m2._2))
		if(i_fdiff==0.0) {
			m1._1.compare(m2._1)
		} else {
			i_fdiff
