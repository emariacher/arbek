...
class CheckEnglishLanguageMostCurrentCombinations2(km: KbdMatrix,
		val i_ghost: Int, ts_quartets: TreeSet[Quartet]) {
	...
	km.L.myPrintln("*f_total*"+CheckEnglishLanguageMostCurrentCombinations2.f_total+
	...
	if(ts_occurrences.until(("",0.5f)).isEmpty) {
		km.L.myErrPrintln(ts_occurrences.until(("",0.02f)).mkString("Natural English language triplet Occurence<pre>\n(triplet,Occurence in %)\n  ","\n  ","\n"))
	} else {
		km.L.myPrintln(ts_occurrences.until(("",0.02f)).mkString("Natural English language triplet Occurence\n(triplet,Occurence in %)\n  ","\n  ","\n"))
		km.L.myErrPrintln(ts_occurrences.until(("",0.5f)).mkString("Natural English language triplet Occurence<pre>\n(triplet,Occurence in %)\n  ","\n  ","\n"))
	}
	km.L.myErrPrintln("\n</pre></td></tr>");
	...
	def getOccurrence(s: String): Float = {
		...
		km.L.myPrintln("  sum["+s+"]="+f_sum+" "+(f_sum*100/CheckEnglishLanguageMostCurrentCombinations2.f_total))
	...
	def checkSpecificGamingTriplet(ts: SortedSet[String], ls: ListSet[String], s_mustShould: String, i_ghost: Int) {
		...
		km.L.myErrPrintln(ts.intersect(ls).toString)
		km.L.myErrPrintln("\n</pre></td></tr>");	
	}
}
