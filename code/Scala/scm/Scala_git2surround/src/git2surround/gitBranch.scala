...
class gitBranch(val gitRepos: GitRepository, val s_branch: String) {
	...
	var i_backupedTag = 0;
	L.myPrintln("************************************************************************************************"); 
	L.myErrPrintln("</pre><h2>***"+this.getClass.getName+"(repository: "+s_gitRepos+", branch: "+s_branch+")****************************************</h2><pre>"); 
	L.myPrintln("************************************************************************************************"); 
	val s_dir = g2s.s_workingDirectory + File.separatorChar + s_gitRepos.substring(s_gitRepos.lastIndexOf("/")+1);
	
	// checkout branch
	i_rc = ScalaBatsh.execrcdir(L, 19, i_rc, "git branch",s_dir)._1;
	i_rc = ScalaBatsh.execrcdir(L, 20, i_rc, "git checkout "+s_branch,s_dir)._1;
	val gitbranch = ScalaBatsh.execrcdir(L, 21, i_rc, "git branch",s_dir);
	i_rc = gitbranch._1;
	val l_branch = gitbranch._2.filter(_.indexOf("* ")==0);
	L.myErrPrintln(this.getClass.getName+l_branch.mkString("</pre>current branch:<ul>\n  <li>","</li>\n  <li>","</li></ul><pre>"));
	
	// build tag list
	if(i_rc == errorCode.OK) {
		val gitlog = ScalaBatsh.execrcdir(L,28,i_rc,"\ngit log --decorate --date=iso",s_dir);
		i_rc = gitlog._1;
	...
	if(i_rc == errorCode.OK) {
		L.myErrPrintln(this.getClass.getName+ll_taggedCommits.mkString("</pre>l_taggedCommits:<ul>\n  <li>","</li>\n  <li>","</li></ul><pre>"));
		if(!ll_commits.isEmpty) {
			L.myErrPrintln("</pre>last commit: "+ll_commits.last.toString+"<pre>");
		};
		if(!ll_taggedCommits.isEmpty) {
			L.myErrPrintln("</pre>last tagged commit: "+ll_taggedCommits.last.toString+"<pre>");
		};
	...
	L.myErrPrintln("  "+this.getClass.getName+" "+s_gitRepos+"__"+s_branch+", rc="+i_rc);
	
	override def toString(): String = {
		L.myPrintln(this.getClass.getName+"****ll_commits:\n"+ll_commits.mkString("\n  "));
		var s = "";
		i_rc match {
		...
		s = s + s_gitRepos + "---"+s_branch+", rc: " + i_rc+" {"+errorCode.nameOf(i_rc)+"}";
		if(!ll_commits.isEmpty && !ll_taggedCommits.isEmpty) {
			L.myPrintln(this.getClass.getName+"****last Commit: ["+ll_taggedCommits.last+"], "+ll_commits.indexOf(ll_taggedCommits.last));
