...
object Git2Surround {
	...
	def checkDirExistsGitInited(L: MyLog, s_dir: String): String = {
			...
			ScalaBatsh.cleanDirectory(L, 33, parentDir, dir);
			ScalaBatsh.createDirectory(L, 34, parentDir, dir);
			checkException(L, ScalaBatsh.exec(L,"ls "+s_dir)._1, s_dir+" is not a valid directory.");
			ScalaBatsh.execrcdir(L, 35, errorCode.OK, "git init", s_dir);
			checkException(L, ScalaBatsh.execrcdir(L, 38, errorCode.OK, "git status --porcelain", s_dir)._1, s_dir+" is not git inited.");
...
//	val s_uid_pwd = l_uid_pwd(0) + ":"+ List.fromString(l_uid_pwd(1),' ').reverse.mkString("","","");
	...
	val l_lsbranch = ScalaBatsh.execrcNoPwd(L,i_rc,"sscm lsbranch -p"+sscmMainLine+" -a"+s_sscmAccess, s_uid_pwd)._2;
