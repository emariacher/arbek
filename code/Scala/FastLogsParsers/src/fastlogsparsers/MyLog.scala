...
object MyLog {
	...
	def printZisday(zisday: Calendar, fmt: String): String = new String(new 
	SimpleDateFormat(fmt).format(zisday.getTime))
...
class MyLog(s_title: String, fil: File,errExt: String) {
	...
	val errfileName = working_directory + "out_"+name_no_ext.replaceAll("\\W", "_")+"_" + 
...
class gridpanel(rows0: Int, cols0: Int, l_parameters: List[(String,String,Any)]) extends GridPanel(rows0, cols0){
	...
	def getFromPanel(): List[(String,String)] = {  
	  ...
	  l_parameters.foreach((field: (String,String,Any)) => l_output = l_output ::: 
	  List(getFromField(field)))
	  l_output
	}
