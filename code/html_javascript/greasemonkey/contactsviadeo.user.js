...
// ==UserScript==
// @name          Contacts Viadeo
// @namespace     http://mariacr.multimania.com/scripts
// @description	  Affiche sur une seule page tous mes contacts Viadeo
// @include       http://www.viadeo.com/contacts/contactsdirects/*
	...
	if (window.confirm("Finished")) {
		...
		for (var i = 0; i < resultpage.length; i++) {
			if(resultpage[i].length>4) {
				outputToTab('<html><head><title>Tous mes contacts Viadeo['+i+']</title></head><body>'+resultpage[i].replace(new RegExp('</table><table>', "g"),'')+'<body><html>');
			}
		}
		fifo_initialise();
...
function searchUrls(numofpage,mxresultpage) {
	...
	for (var i = 1; i < numofpage; i++) {
		fifo_push('http://www.viadeo.com/contacts/contactsdirects/index.jsp?pageNumber='+i);
	}
...
function getNumberOfContacts() {
	var txt, urlStr='http://www.viadeo.com/contacts/contactsdirects/index.jsp?pageNumber=1';
	request = {
		method:'GET',
		url: urlStr,
		headers: {
			...
			if (responseDetails.readyState == 4) {
				...
				if (responseDetails.status == 200) {
					...
					' ' + responseDetails.statusText + ' ' + responseDetails.responseText.length);
					  ...
					  var number=responseDetails.responseText.substring(index1+zob.length,index2).replace(new RegExp('&nbsp;', "g"),'');
					  var numofcontacts = parseInt(number);
					  li_dbg(numofcontacts+ ' [' +number+']');
