...
public class UtilsDate {
	...
	Calendar get1erJour(int annee, int mois) {
		...
		c.set(annee, mois, 1);
		while (c.get(Calendar.DAY_OF_WEEK) != today.get(Calendar.DAY_OF_WEEK)) {
			c.roll(Calendar.DAY_OF_MONTH, true);
		}
		return c;
	}
	 ...
	 *@return    Description of the Return Value
	 */
	Calendar get1erLundi(Calendar u) {
		Calendar c = (Calendar) u.clone();
		while (c.get(Calendar.DAY_OF_WEEK) != Calendar.MONDAY) {
			c.roll(Calendar.DAY_OF_MONTH, true);
		}
		return c;
	 ...
	 *
	 *@param  c               Description of the Parameter
	 *@param  durationInDays  Description of the Parameter
	 *@param  now             Description of the Parameter
	 *@return                 Description of the Return Value
	 */
	boolean changedLastPeriod(Calendar now, Calendar c, int durationInDays) {
		Calendar before = (Calendar) now.clone();
		before.add(Calendar.DAY_OF_MONTH, (-1) * durationInDays);
		if (c.after(before) & c.before(now)) {
			return true;
		}
		return false;
	}
	...
	/**
	 *  Gets the today attribute of the UtilsDate object
	 *
	 *@param  now  Description of the Parameter
	 *@return      The today value
	 */
	boolean isToday(Calendar now) {
		Calendar today = Calendar.getInstance();
		return ((now.get(Calendar.DAY_OF_YEAR) == today.get(Calendar.DAY_OF_YEAR)) &&
