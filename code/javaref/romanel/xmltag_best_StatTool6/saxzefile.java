...
public class saxzefile extends DefaultHandler implements LexicalHandler, DefVar {
  ...
  Log L;
  int cpt                      = 0;
  List<XmlTag> xt_active        = new ArrayList<XmlTag>();
  ...
  Attributes attrs)
    ...
    indentLevel++;
    cpt++;
    xt_active.add(indentLevel,new XmlTag(xt_active.get(indentLevel - 1)));
    xt_active.get(indentLevel - 1).addChild(xt_active.get(indentLevel));
    nl();
    ...
    emit("<" + eName);
    xt_active.get(indentLevel).name(eName);
    if(attrs != null) {
      for(int i = 0; i < attrs.getLength(); i++) {
        ...
        emit("\"");
        xt_active.get(indentLevel).addAttribute(attrs.getLocalName(i), attrs.getValue(i));
      }
    }
    if(trace) {
  ...
  public void characters(char buf[], int offset, int len)
    ...
    /*if(s.matches("\\S+")) {*/
    if(s.trim().length() > 0) {
      xt_active.get(indentLevel).addNoTag(s.trim());
    }
    /* ******** ajoute tes machins ici: fin */
    if(s.trim().length() > 0) {
      emit(s);
