...
public class JTreeMapExample extends JFrame implements ActionListener {
	 ...
	 * Constructor
	 */
	public JTreeMapExample(final String[] args) {
	...
	public static void main(final String[] args) {
		final JTreeMapExample example = new JTreeMapExample(args);
	...
	protected void createColorProviders() {
		...
		colorProviders.put("HSB linear", new HSBTreeMapColorProvider(jTreeMap,
				HSBTreeMapColorProvider.ColorDistributionTypes.Linear, Color.BLUE, Color.GREEN));
		colorProviders.put("HSB SquareRoot", new HSBTreeMapColorProvider(jTreeMap,
				HSBTreeMapColorProvider.ColorDistributionTypes.SquareRoot, Color.MAGENTA, Color.RED));
	 ...
	 * @throws Exception
	 */
	private void initGUI(final String[] args) throws Exception {
		...
		File defvardiff = new File("JTreeMap");
		if(defvardiff.canRead()) {
			BufferedReader inputf = new BufferedReader(new FileReader(defvardiff));
			String linef;
			int cpt       = 0;
			while((linef = inputf.readLine()) != null) {
				switch (cpt) {
				case 0:
					lastDirectory = new File(linef);
					break;
				}
				cpt++;
			}
    ...
    outputf.write((new String(chooser.getCurrentDirectory().toString() + "\n")).getBytes());
    outputf.close();
