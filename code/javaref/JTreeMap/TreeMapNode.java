...
public class TreeMapNode extends DefaultMutableTreeNode {
	...
	private int y;
	private static float INVALID =(float)99.99;
	private float hue=INVALID;
	...
	public TreeMapNode(final String label, final double weight, final Value value, final double hue) {
		super(label);
		// the weight must be positive
		this.weight = Math.abs(weight);
		this.value = value;
	...
	public TreeMapNode(final String label, final double weight, final Value value, final double hue, final String details) {
		super(label);
		// the weight must be positive
		this.weight = Math.abs(weight);
		this.value = value;
	...
	public TreeMapNode(final String label, final double weight, final Value value, TreeMapNode tmn) {
		super(label);
		// the weight must be positive
		this.weight = Math.abs(weight);
		this.value = value;
	...
	public Color getColor(final Value zvalue, ColorProvider cp) {
		if (hue==INVALID){
			if(parent!=null){
				return ((TreeMapNode)parent).getColor(zvalue, cp);
			} else {
				return cp.getColor(zvalue);	
			}
		}
		return cp.getColorz(zvalue, hue);
	}
	...
	public Color getFontColor() {
		if((hue==INVALID) || (value.getValue()>0)){
			return Color.WHITE;
		} else {
