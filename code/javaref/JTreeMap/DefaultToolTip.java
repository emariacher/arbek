...
//		final int height = getFontMetrics(this.labelFont).getHeight() + getFontMetrics(this.valueFont).getHeight();
	...
	public void paint(final Graphics g) {
		if (this.jTreeMap.getActiveLeaf() != null) {
			...
			String details = this.jTreeMap.getActiveLeaf().getLabelValue();
			
			int iprev=0;
			int u=0;
			int inext=details.indexOf(',',iprev);
			if(inext<0){
				g2D.drawString(details, TOOLTIP_OFFSET, this.getHeight() - TOOLTIP_OFFSET);
			}
			while(inext>0){
				g2D.drawString(details.substring(iprev,inext), TOOLTIP_OFFSET, 30 + (u*11));
				iprev=inext+1;
				u++;
				inext=details.indexOf(',',iprev);
			}
		}
	}
}
