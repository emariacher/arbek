...
public class HSBTreeMapColorProvider extends ColorProvider {
    ...
    public float getHue(final Color color){
...
//          return Color.getHSBColor(positiveHue, positiveSaturation, (float) (adjustValue(val) / range));
          return Color.getHSBColor(positiveHue, (float) (adjustValue(val) / range), (float)1.0);
      }
...
//      return Color.getHSBColor(negativeHue, negativeSaturation, (float) (adjustValue(val) / range));
      return Color.getHSBColor(negativeHue, (float) (adjustValue(val) / range), (float)1.0);
  }
    public Color getColorz(Value value, float hue) {
      // Figure out the current range of colors, map that range into a scale
      // from 0 to 1,
      // using the specified distribution type
      if (maxValue == null || minValue == null) {
      ...
      final double min = this.minValue.getValue();
      double val = (value != null ? value.getValue() : 0.00);
      val+=1; // no blank so that every leaf seems lightly colored when value == 0
      if(val>max){
      	val=max; // if val was already at max, decrease it to max
      }
      ...
      if (val >= 0) {
          ...
          range = adjustValue(range);
//          return Color.getHSBColor(positiveHue, positiveSaturation, (float) (adjustValue(val) / range));
          return Color.getHSBColor(hue, (float) (adjustValue(val) / range), (float)1.0);
      }
