...
class Log {
	...
	String initLogFiles() throws Exception {
		...
		logfile = new MyFile(working_directory + File.separatorChar+"logfile_" + 
				printToday("ddMMMyy_HH_mm") + ".log");
		files.add(logfile);
		errfile = new MyFile(working_directory + File.separatorChar+"errfile_" + 
				printToday("ddMMMyy_HH_mm") + ".log");
		files.add(errfile);
		System.err.println("initLogFiles():" + working_directory);
		t_start = Calendar.getInstance();
		myPrintln("t_start: " + printZisday(t_start,"ddMMMyy_HH_mm_ss_SSS"));
		return working_directory;
	}


	static	String printToday(String fmt) throws Exception {
		Calendar today  = Calendar.getInstance();
		return printZisday(today, fmt);
	}

	static	String printToday() throws Exception {
		return printToday("ddMMMyy");
	}

	static	String printZisday(Calendar zisday, String fmt) throws Exception {
		return printZisday(zisday.getTime(), fmt);
	}

	static	String printZisday(Date date, String fmt) throws Exception {
	...
	void closeFiles() throws Exception {
		Calendar t_end = Calendar.getInstance();
		myPrintln("t_end: " + printZisday(t_end,"ddMMMyy_HH_mm_ss_SSS"));
	...
	String getJarFileCompilationDate() throws URISyntaxException {
        File jarFile = new File
        (this.getClass().getProtectionDomain().getCodeSource().getLocation().toURI());
        return "["+jarFile.getName() + "] Version: " + new Date(jarFile.lastModified()).toString();
}
