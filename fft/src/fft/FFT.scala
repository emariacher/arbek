...
object FFT extends App {
  ...
  def lowpass(l: List[String], w: Int): List[Double] = l.map(_.split(";").toList.last.toString).tail.filter(_.length > 2).map(_.toDouble).sliding(w).toList.map(_.sum / w)
  def dumpLowPass(fili: File, lw: List[Int], drop: Int, take: Int) {
    ...
    lw.foreach(w => {
      val lp = lowpass(l, w)
      val x = (1 to lp.length).map(_.doubleValue).drop(drop).take(take)
      plot(x.toArray, lp.drop(drop).take(take).toArray, MyPlotUtils.getColor(hue), "lowpass" + w)
      hue += incHue
    })
  }
  ...
  dumpLowPass(f, List(2, 5, 10), 10, 5000)
  myErrPrintln("Done!")
}