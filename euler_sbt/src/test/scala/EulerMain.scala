...
class EulerMain extends FlatSpec with Matchers {
  ...
  "Euler33" should "be OK" in {
    ...
    val paspremiers100 = (10 to 99).filter(i => !premiers100.contains(i))
    ...
    val z = paspremiers100.map(i => {
      paspremiers100.dropWhile(_ < i).filter(j => i.toString.toList.intersect(j.toString.toList).nonEmpty).map(j => {
        (i, j, i.toDouble / j.toDouble)
      })
    })
    println(z.flatten.groupBy(_._3).filter(g => g._2.length > 1).toList.sortBy(_._1)(Ordering.Double.TotalOrdering).mkString("\n"))
